...
class xnewsletterRequest
{
    ...
    {
        return (string)Request::getVar($name, $default, $hash, 'string', PUBLISHER_REQUEST_ALLOWRAW);
    }
     ...
     * @param bool   $overwrite If true and an existing key is found, the value is overwritten, otherwise it is ingored
     */
     ...
     * @param bool  $globalise True if the array is to be added to the GLOBALS
     */
    public static function _cleanArray(&$array, $globalise = false)
    {
        static $banned = ['_files', '_env', '_get', '_post', '_cookie', '_server', '_session', 'globals'];
        foreach ($array as $key => $value) {
    ...
    {
        $value = is_array($value) ? array_map(['xnewsletterRequest', '_stripSlashesRecursive'], $value) : stripslashes($value);
...
class xnewsletterFilterInput
{
    ...
    public function clean($source, $type = 'string')
    {
        ...
        switch (mb_strtoupper($type)) {
            ...
            case 'ARRAY':
                ...
                $result = is_array($source) ? (array)$source : [$source];
                break;
            case 'PATH':
    ...
    public function checkAttribute($attrSubSet)
    {
        ...
        return (((false !== mb_strpos($attrSubSet[1], 'expression')) && 'style' === $attrSubSet[0])
                || (false !== mb_strpos($attrSubSet[1], 'javascript:'))
                || (false !== mb_strpos($attrSubSet[1], 'behaviour:'))
                || (false !== mb_strpos($attrSubSet[1], 'vbscript:'))
                || (false !== mb_strpos($attrSubSet[1], 'mocha:'))
                || (false !== mb_strpos($attrSubSet[1], 'livescript:')));
    }
    ...
    protected function _cleanTags($source)
    {
        ...
        while (false !== $tagOpen_start) {
            ...
            if (false === $tagOpen_end) {
                $postTag       = mb_substr($postTag, $tagOpen_start + 1);
            ...
            while (false !== $currentSpace) {
                ...
                if (false !== mb_strpos($fromSpace, '=')) {
                     ...
                     * till the next space
                     */
                    if ((false !== $openQuotes) && (false !== mb_strpos(mb_substr($fromSpace, $openQuotes + 1), '"'))) {
        ...
        for ($i = 0, $iMax = count($attrSet); $i < $iMax; ++$i) {
            ...
            if ($attrSubSet[1]) {
                ...
                if (("'" === mb_substr($attrSubSet[1], 0, 1))
                    ...
                    $attrSubSet[1] = mb_substr($attrSubSet[1], 1, -2);
                }
                // strip slashes
