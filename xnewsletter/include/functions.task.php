...
function xnewsletter_createTasks($op, $letter_id, $xn_send_in_packages, $xn_send_in_packages_time)
{
    ...
    if ($op == 'send_test') {
        ...
        while ($subscr = $GLOBALS['xoopsDB']->fetchArray($subscrs)) {
    ...
    if (count($recipients) == 0) {
        ...
        foreach ($recipients as $subscr_id) {
            ...
            if ($xn_send_in_packages > 0) {
                if ($recipentsCounter == $xn_send_in_packages) {
                    $task_starttime = $task_starttime + (60 * $xn_send_in_packages_time); // minutes
                    $recipentsCounter = 0; // reset counter
                }
            }
            ++$recipentsCounter;
            // create task list item
            $sql = "INSERT INTO `{$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}`";
            ...
            if (!$GLOBALS['xoopsDB']->queryF($sql)) {
                // create protocol
                $protocolObj = $xnewsletter->getHandler('protocol')->protocol($letter_id, $subscr_id, _AM_XNEWSLETTER_TASK_ERROR_CREATE, _XNEWSLETTER_PROTOCOL_STATUS_ERROR_CREATE_TASK,  array(), false);
                unset($protocolObj);
                return false;
            } elseif ($op != 'send_test') {
                // update letter
 ...
 * @return mixed|string
 */
function xnewsletter_executeTasks($xn_send_in_packages = 0, $letter_id = 0)
{
    ...
    $sql = "SELECT `task_letter_id`";
    $sql .= " FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
    if (!empty($letter_id)) {
    ...
    $task_letter_ids = array();
    while ($record = $GLOBALS['xoopsDB']->fetchArray($result)) {
        $task_letter_id = $record['task_letter_id'];
        $taskLetterObj = $xnewsletter->getHandler('letter')->get($task_letter_id);
        if (count($taskLetterObj) == 0) {
            return _AM_XNEWSLETTER_SEND_ERROR_NO_LETTERID;
        }
        // read categories
        $letter_cats = $taskLetterObj->getVar('letter_cats');
        if ($letter_cats == '') {
            //no cats
            return _MA_XNEWSLETTER_LETTER_NONEAVAIL;
        }
        // read data of account
        $letter_account = $taskLetterObj->getVar('letter_account');
        if ($letter_account == '' || $letter_account == 0) {
            return _MA_XNEWSLETTER_ACCOUNTS_NONEAVAIL;
        }
        $task_letter_ids[] = $task_letter_id;
    }

    // execute task letter by letter
    foreach ($task_letter_ids as $task_letter_id) {
        $taskLetterObj = $xnewsletter->getHandler('letter')->get($task_letter_id);
        // read categories
        $letter_cats = $taskLetterObj->getVar('letter_cats');
        // read data of account
        ...
        // letter data
        $letter_title = $taskLetterObj->getVar('letter_title');
        $letter_content = $taskLetterObj->getVar('letter_content', 'n');
        $letterTpl->assign('letter_id', $task_letter_id); // new from v1.3
        ...
        $attachmentAslinkCriteria->add(new Criteria('attachment_letter_id', $task_letter_id));
        ...
        // get emails of subscribers
        $recipients = array();
        $sql_tasklist = "SELECT `task_id`, `task_subscr_id`";
        $sql_tasklist .= " FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
        $sql_tasklist .= " WHERE ((`task_letter_id` = {$task_letter_id}) AND (`task_starttime` < " . time() . "))";
        if (!$result = $GLOBALS['xoopsDB']->query($sql_tasklist)) {
            return $task_letters->getErrors();
        }
        $recipients = array();
        while ($task = $GLOBALS['xoopsDB']->fetchArray($result)) {
            $task_subscr_id = $task['task_subscr_id'];
            $task_id = $task['task_id'];
            if ($task_subscr_id == 0) {
                // multiple test emails support
                $addresses = explode(';', $taskLetterObj->getVar('letter_email_test'));
                ...
                $sql_subscr = "SELECT *";
                $sql_subscr .= " FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_subscr')}";
                $sql_subscr .= " WHERE `subscr_id` = {$task_subscr_id}";
                if (!$task_subscrs = $GLOBALS['xoopsDB']->query($sql_subscr)) {
                    return $task_subscrs->getErrors();
                }
                $subscr = $GLOBALS['xoopsDB']->fetchArray($task_subscrs);
                $recipients[] = array(
                    'task_id'           => $task_id,
        ...
        $attachmentAsattachmentCriteria->add(new Criteria('attachment_letter_id', $task_letter_id));
        ...
        foreach ($attachmentObjs as $attachment_id => $attachmentObj) {
            $attachmentsPath[] = XOOPS_UPLOAD_PATH . $xnewsletter->getConfig('xn_attachment_path') . $task_letter_id . '/' . $attachmentObj->getVar('attachment_name');
        }
        // send email
        try {
            ...
            foreach ($recipients as $recipient) {
                $task_subscr_id = $recipient['subscriber_id'];
                // subscr data
                ...
                preg_match('/db:([0-9]*)/', $taskLetterObj->getVar('letter_template'), $matches);
                if (isset($matches[1]) && ($templateObj = $xnewsletter->getHandler('template')->get((int)$matches[1]))) {
                    ...
                    $template = $template_path . $taskLetterObj->getVar('letter_template') . '.tpl';
                ...
                $mail->ClearAddresses();
                // delete just executed task
                $sql_delete = "DELETE";
                $sql_delete .= " FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
                $sql_delete .= " WHERE `task_id` = {$recipient['task_id']}";
                $result = $GLOBALS['xoopsDB']->queryF($sql_delete);
                // create protocol
                $protocolObj = $xnewsletter->getHandler('protocol')->protocol($task_letter_id, $task_subscr_id, $protocol_status, $protocol_status_str_id, $protocol_status_vars, $protocol_success);
                unset($protocolObj);
            }
            unset($mail);
        } catch (phpmailerException $e) {
             // error messages from PHPMailer
            $protocol_status = _AM_XNEWSLETTER_SEND_ERROR_PHPMAILER . $e->errorMessage(); // old style
            ...
            $protocol_status = _AM_XNEWSLETTER_SEND_ERROR_PHPMAILER . $e->getMessage(); // old style
            $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_ERROR_PHPMAILER; // new from v1.3
            $protocol_status_vars = array('%error' => $e->getMessage()); // new from v1.3
            ++$errorsCounter;
            $protocol_success = false;
        }
    }
    ...
    if ($errorsCounter > 0) {
        $protocol_status = xnewsletter_sprintf(_AM_XNEWSLETTER_SEND_ERROR_NUMBER, array('%e' => $errorsCounter, '%t' => $tasksCounter)); // old style
        $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_ERROR_SEND_COUNT; // new from v1.3
        $protocol_status_vars = array('%error_count' => $errorsCounter, '%total_count' => $tasksCounter); // new from v1.3
        $protocol_success = false;
    } else {
        $protocol_success = true;
        if ($tasksCounter > 0) {
            $protocol_status = xnewsletter_sprintf(_AM_XNEWSLETTER_SEND_SUCCESS_NUMBER, array('%t' => $tasksCounter)); // old style
            $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_OK_SEND_COUNT; // new from v1.3
            $protocol_status_vars = array('%total_count' => $tasksCounter); // new from v1.3
        } else {
            $protocol_status = ''; // old style
            $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_EMPTY; // new from v1.3
            $protocol_status_vars = array(); // new from v1.3
        }
    }
    // create protocol
    $protocolObj = $xnewsletter->getHandler('protocol')->protocol($letter_id, 0, $protocol_status, $protocol_status_str_id, $protocol_status_vars, $protocol_success);
    $protocol_status = $protocolObj->getStatusString();
    unset($protocolObj);
