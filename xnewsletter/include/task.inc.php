    ...
    if (count($recipients) == 0) {
        ...
        foreach ($recipients as $subscr_id) {
            ...
            if (!$GLOBALS['xoopsDB']->queryF($sql)) {
                ...
                $protocolObj->setVar('protocol_status', _AM_XNEWSLETTER_TASK_ERROR_CREATE); // old style
                $protocolObj->setVar('protocol_status_str_id', _XNEWSLETTER_PROTOCOL_STATUS_ERROR_CREATE_TASK); // new from v1.3
...
function xnewsletter_executeTasks($xn_send_in_packages, $letter_id = 0)
{
    ...
    $sql = "SELECT `task_letter_id` FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
    if ($letter_id > 0) {
        $sql .= " WHERE (`task_letter_id`={$letter_id})";
    }
    $sql .= " GROUP BY `task_letter_id`";
    if (!$task_letters = $GLOBALS['xoopsDB']->query($sql)) {
        return _AM_XNEWSLETTER_SEND_ERROR_NO_LETTERID;
    }

    while ($task_letter = $GLOBALS['xoopsDB']->fetchArray($task_letters)) {
        ...
        // get emails of subscribers
        $recipients   = array();
        $sql_tasklist = "SELECT `task_id`, `task_subscr_id` FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
        $sql_tasklist .= " WHERE ((`task_letter_id`= {$letter_id}) AND (`task_starttime` < " . time() . "))";
        if (!$task_letters = $GLOBALS['xoopsDB']->query($sql_tasklist)) {
        ...
        while ($task_letter = $GLOBALS['xoopsDB']->fetchArray($task_letters)) {
            ...
            if ($subscr_id == 0) {
                ...
                $sql_subscr = "SELECT * FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_subscr')}";
        ...
        try {
            ...
            foreach ($recipients as $recipient) {
                ...
                $sql_delete = "DELETE FROM {$GLOBALS['xoopsDB']->prefix('xnewsletter_task')}";
    ...
    if ($count_err > 0) {
        $protocol_status = xnewsletter_sprintf(_AM_XNEWSLETTER_SEND_ERROR_NUMBER, array('%e' => $count_err, '%t' => $count_total)); // old style
        $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_ERROR_SEND_COUNT; // new from v1.3
        $protocol_status_vars = array('%error_count' => $count_err, '%total_count' => $count_total); // new from v1.3
        $protocol_success = false;
    } else {
        $protocol_success = true;
        if ($count_total > 0) {
            $protocol_status = xnewsletter_sprintf(_AM_XNEWSLETTER_SEND_SUCCESS_NUMBER, array('%t' => $count_total)); // old style
            $protocol_status_str_id = _XNEWSLETTER_PROTOCOL_STATUS_OK_SEND_COUNT; // new from v1.3
            $protocol_status_vars = array('%total_count' => $count_total); // new from v1.3
        } else {
